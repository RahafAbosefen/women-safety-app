...
//   onPress={() => {
//     console.log("Opening company details:", item.id);

//     router.push(`/userTabs/company-details/${item.id}` as any);
//   }}
// >
...
export default function ContactUsScreen() {
  ...
  useEffect(() => {
    ...
    const saveCompaniesToSQLite = async () => {
      ...
      for (const company of onlineCompanies as Company[]) {
        await db.runAsync(
          `INSERT OR REPLACE INTO companies 
          (id, name, type, phone, email) 
          VALUES (?, ?, ?, ?, ?);`,
          String(company.id),
          String(company.name || company.companyName || ""),
          String(company.type || company.serviceType || ""),
          String(company.phone || company.phoneNumber || ""),
          String(company.email || "")
        );
      }
  ...
  const displayedCompanies: Company[] =
    companies.length > 0 ? companies : ((onlineCompanies || []) as Company[]);
  ...
  const renderCompany = useCallback(
    ({ item }: { item: Company }) => {
      const companyName = item.name || item.companyName || "Company";
      const companyType = item.type || item.serviceType || "Company";
  ...
  if (isLoading && displayedCompanies.length === 0) {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color={AppColors.primary} />
      </View>
  ...
  if (isError && displayedCompanies.length === 0) {
    return (
      <View style={styles.loadingContainer}>
        <Text style={styles.errorText}>Something went wrong</Text>
      </View>
